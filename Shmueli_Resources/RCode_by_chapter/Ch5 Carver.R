...
## My note:  training & test are random 1000, possibly with overlap
## Text code does not set a seed!!
...
# training residuals
resid_t <- pred_t - toyota.corolla.df[training,]$Price
hist(resid_t, breaks=150)

# validation
accuracy(pred_v, toyota.corolla.df[validation,]$Price)
resid_v <- pred_v - toyota.corolla.df[validation,]$Price
hist(resid_v, breaks=150)
