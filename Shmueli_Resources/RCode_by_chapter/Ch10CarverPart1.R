...
#### Figure 10.4
##
#  Read in airline delays data again
...
agg <- aggregate(delays.df$isDelay, 
    by = list(delays.df$DAY_WEEK, delays.df$CARRIER, delays.df$ORIGIN), 
                 FUN = mean, na.rm = TRUE)
# m <- melt(agg)
names(agg)[1:3] <- c("DAY_WEEK", "CARRIER", "ORIGIN")

agg$DAY_WEEK <- factor(agg$DAY_WEEK, labels = c("Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"))
...
ggplot(agg, aes(y = CARRIER, x = DAY_WEEK, fill = x)) +  geom_tile() + 
