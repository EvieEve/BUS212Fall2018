...
boston.housing.df <- read.csv("Data/BostonHousing.csv", header = TRUE) 
...
library(reshape) 
boston.housing.df <- read.csv("Data/BostonHousing.csv")
# create bins of size 1
boston.housing.df$RM.bin <- .bincode(boston.housing.df$RM, c(1:9)) 
...
cereals.df <- read.csv("Data/Cereals.csv") 
...
############################
#
# Now create equivalents of Table 4.10 to 4.12 for Airbnb numeric columns
library(tidyverse)

#### Table 4.10


airbnb.df <- read_csv("Data/Boston Listings 2018 Case 1 Main v3.csv")
glimpse(airbnb.df)  # find content-relevant numeric columns

keepnum <- c(12,13,17:29)
airnum.df <- select(airbnb.df, keepnum)
head(airnum.df, 10)

# explore some correlations
library(GGally)
ggcorr(airnum[, c(3:7)], label=TRUE, cex=3)

ggpairs(airnum, columns= c(3:5, 8:9), upper = list(continuous = wrap("cor", size = 4)))

# compute PCs on all numeric dimensions

pcsa <- prcomp(na.omit(airnum.df)) 
summary(pcsa)
...
pcsa.cor <- prcomp(na.omit(airnum.df), scale. = T)
summary(pcsa.cor)
